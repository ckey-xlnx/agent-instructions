...
name: ckey-review
description: >
  Christopher Key's custom automated review process for ReviewBoard review
  requests: summarize pending reviews, analyze a review request against the
  coding/repo guidelines, submit interactive feedback, and learn from feedback
  patterns. Use when asked to review a ReviewBoard request, summarize pending
  reviews, analyze a diff/review request, suggest review comments, post review
  feedback, reply to review comments, or update guidance based on review
  feedback. This is the ReviewBoard-specific process; for GitHub PRs use the
  generic review tooling instead.
---

# Automated Review Request Feedback Process

This is Christopher Key's custom review process, currently coupled to ReviewBoard.
(Future refactor: split the generic review + learning process from the
ReviewBoard-specific integration.)
