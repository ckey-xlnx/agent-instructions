...
Concrete procedures now live as Claude Code skills under `.claude/skills/`, which load on demand rather than always consuming context. For example, SimNow release installation is in `.claude/skills/simnow-install-release/`. Add new operational procedures as skills there (one level deep: `.claude/skills/<name>/SKILL.md` — subdirectories are not discovered).
